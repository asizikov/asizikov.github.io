...
Let’s say that we have a URL like that

nonexisting-api.github.com/v1/rating/user_name
...
Assuming that we’re building the mobile app, and taking into account that our data is not business critical we can consider the following requirements:
...
Generally it’s not a great idea to re-render the UI for no reason. To avoid an unnecessary invocation of the OnNext delegate we’re going to use [DistinctUntilChanged](https://msdn.microsoft.com/en-us/library/system.reactive.linq.observable.distinctuntilchanged(v=vs.103).aspx) operator.
...
* [ReactiveX home](https://reactivex.io/)
